<script lang="ts">
	...
	async function summarizeFile() {
		...
		try {
			...
			const response = await fetch('/api/parse-summarize-openai', {
				method: 'POST',
				body: formData
			});
			...
			toast.error('Error summarizing file:', error);
...
<div class="container mx-auto py-8">
	<div class="mb-6 flex max-w-screen-sm flex-col gap-2">
		<h1 class="text-3xl font-bold">AI File Summarizer</h1>
		<p>
			This is the simplest workflow. It uses a SvelteKit API endpoint to upload the file, parse it,
			to upload the file, parse it, and then pass it to an AI model to summarize it. It returns the
